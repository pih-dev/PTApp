...
The header shows the PTApp logo, the app title, and two indicators on the right:

| Control | What it does |
|---------|-------------|
| **Colored dot** | Sync status: green = synced, blue pulsing = syncing, red pulsing = failed (tap to retry). Only appears after first sync. |
| **⋮** | Opens the General panel (language, theme, backup, todos, WhatsApp templates, docs). |

Language (**Ar / En**) and theme (**Lit / Drk**) toggles live at the top of the General panel — tap the **⋮** button to open it. Both choices are saved and persist across sessions.

### Sync Status

The colored dot next to the ⋮ button tells you if your data is synced to the cloud:

| Color | Meaning |
|-------|---------|
| **Green** | All changes synced successfully |
| **Blue (pulsing)** | Sync in progress |
| **Red (pulsing)** | Sync failed — **tap the dot to retry** |

If the dot is red, your changes are safe locally — they just haven't reached the cloud yet. Tap the red dot to retry. If the internet is down, it will sync automatically next time you open the app with a connection.

### Debug Panel

**Long-press** the ⋮ button (hold for ~1 second) to show a diagnostic panel with version, sync state, session count, and other technical info. Useful for troubleshooting. Long-press again to hide it.
...
**Sync status indicator:** A colored dot in the header shows whether your data is synced to the cloud. Green = synced, blue = syncing, red = failed. If red, tap it to retry. You'll never wonder if your changes made it to the cloud.

**Data protection:** The app now prevents stale data from overwriting newer data. If your phone can't reach the cloud on startup, it won't push old data — it waits until it can fetch the latest version first. This prevents the kind of data loss that happened on Apr 13.

**Debug panel:** Long-press the ⋮ menu button to see technical details: version, sync state, session count, client count, and more. Useful for troubleshooting.

**Cleaner header:** The version number was moved out of the header into the debug panel and General panel. The header now shows just the sync dot and the ⋮ button — cleaner and easier to tap on iPhones.

**Works properly as an app on iPhone:** Adding the app to your Home Screen now opens it in full-screen mode (no Safari bar at the bottom). Your login persists between opens. If you previously added the app and see a Safari bar, delete the Home Screen icon and re-add it from Safari.
...
Tap the **⋮** button in the header to open the General panel.
...
- Data syncs to the cloud automatically (1-second debounce after changes)
- The **sync dot** in the header shows whether the cloud is up to date
- If the cloud can't be reached, your data is safe locally — it syncs when connection returns
- The app **never pushes old data over newer data** — it compares timestamps on startup
...
1. Open the app in **Safari** (not Chrome or another browser)
2. Enter your sync token and tap Connect
3. Tap the **Share** button (square with arrow) → **Add to Home Screen**
4. Open the app from the new Home Screen icon — it runs in full-screen mode

The Home Screen app has its own storage. Your token and data persist between opens. If you see Safari's URL bar at the bottom, delete the icon and re-add it.
