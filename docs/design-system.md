...
## The token contract (v2.18–v2.21) — the full list

Routed here from `CLAUDE.md`, which keeps the RULES and points at this for the inventory.
🔴 **Every skin defines every one of these; `sanity-skins.mjs` fails the build if one is missing.**

| Group | Tokens | Used for |
|---|---|---|
| Inline-style ladder (pre-v2.18, still correct in JSX) | `--t1`…`--t5`, `--sep`, `--card-bg` | opacity ramp over the text colour |
...
🔴 Bundled, never fetched — `src/fonts.css` is GENERATED by `scripts/build_fonts.mjs`; latin subsets
only, so the Arabic build falls back to the platform face by design.

**Contrast is measured against BOTH ends of the ground gradient**, not just `--ground`: the lit end
is where the week strip sits, and a ramp checked only against the dark end passes while the top of
the screen fails. Text ≥4.5, graphics (bar, plate outline) ≥3. That rule was earned — the first
`steel` ramp shipped at 1.4–2.7:1 and was invisible for everyone who picked that skin.
