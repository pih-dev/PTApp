...
The header shows the PTApp logo, the app title, and three controls on the right:

| Control | What it does |
|---------|-------------|
| **Ar / En** | Switches the entire UI between Arabic (RTL) and English (LTR). Defaults to English. |
| **Lit / Drk** | Switches between Light theme (white + blue) and Dark theme (dark + red). Defaults to Dark. |
| **v2.2 ⋮** | Opens the General panel (backup, todos, WhatsApp templates, docs). |

Both language and theme choices are saved — they persist when you close and reopen the app.
...
**Sessions currently in progress have a glow** — a tinted background, colored border, and soft glow so you can instantly see what's happening now (red in dark theme, blue in light theme). All sessions running at the same time get the glow.
...
You can edit both message templates from the General panel (⋮ → WhatsApp Messages):
...
Tap the **v2.2 ⋮** button in the top-right corner of the header to open the General panel.
...
A shared list for notes and reminders. Both the PT and the developer can add, edit, and delete items. Tap an item to edit it inline. Items sync between devices.
