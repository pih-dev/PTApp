...
- **Focus tags reset** — the old tags are cleared since they don't apply to the new type. The new type's tags appear automatically.
...
Filter buttons at the top: **All**, **Scheduled**, **Confirmed**, **Completed**, **Cancelled**.

### Session Info

Each card shows:
- Client name with **#N** (monthly session count for that session's month)
...
1. Open **Home** to see today's overview and upcoming sessions
...
Cancelled sessions stay in the system (visible in the Sessions tab with a "Cancelled" badge). They are never deleted.

---

## Monthly Session Count (#N)

The **#N** number shown next to client names on session cards tracks how many sessions that client has in that calendar month.
...
- On client chips when booking (shows current month count before the new booking)
...
| **Cancelled** | Session was cancelled | (Final state — counted or forgiven) |
...
- No internet connection is needed to use the app (except for WhatsApp links and font loading)
- Data persists between sessions — closing and reopening the browser keeps everything
- **Clearing browser data will erase all app data** — be careful with "Clear browsing data" settings
