...
- [Figure anatomy report](2026-08-24-figure-anatomy-report.md) `docs/2026-08-24-figure-anatomy-report.md` — the 2026-08-24 report HANDED TO THE CCHEALTH SESSION (it owns the figures thread; PTApp measured, did not edit). 🔴 **A prone foot carries the upright sign convention** — `knee-flexion` and `bird-dog` sit at −88°/−80° where every other prone pose is +40°, and archetypes.js records the same inversion already being applied to the SHIN and not the foot. 🔴 **No gate checks the viewBox**: 17 poses across 11 patterns put a joint centre outside the frame; a `FRAME:` check beside `FLOOR:` is proposed. Plus which 5 ROM warnings are real and which 5 are the gate measuring a quantity that does not apply to a support pose
...
- [taste-skill evaluation](design/2026-08-22-taste-skill-evaluation.md) `docs/design/2026-08-22-taste-skill-evaluation.md` — Leonxlnx/taste-skill reviewed for SpotSet: NOT installed (its own scope bars product UI, and we already do most of it); harvest its AI-Tells list for the landing page / store listing / deck, plus `text-wrap`, the em-dash pass and skeleton states
- [Fresh-eyes navigation review, second run](design/2026-08-22-fresh-eyes-navigation-review.md) `docs/design/2026-08-22-fresh-eyes-navigation-review.md` - the stripped-structure attack on the BOTTOM BAR and IA: keep the bar, replace the Sessions tab with the Library, and stop leaving the plate calculator in the middle ground. Two calls await Pierre
- [Roles and the dev surface](design/2026-08-22-roles-and-the-dev-surface-brainstorm.md) `docs/design/2026-08-22-roles-and-the-dev-surface-brainstorm.md` - BRAINSTORM, nothing decided: General is three audiences (product / tester / dev) stacked in one drawer; the honest split is at build time, not a hidden menu item. Elie's hierarchy is already in Postgres via owner_path but nothing reads it yet
...
- [The music engine, and two suites](2026-08-24-music-engine-and-suites.md) `docs/2026-08-24-music-engine-and-suites.md` — why the v2.31 suite sounded synthesized (additive oscillators have no noise, inharmonicity, body or room) and the engine that fixes it: breath-noise flute, Karplus-Strong guitar, inharmonic beating piano, formant sax, dynamic-brightness brass, inharmonic timpani, bowed marcato strings, choir, 8-line FDN room. **Round one (suite2, acoustic) was REJECTED — 'the only difference from the original ones should be real instruments instead of synthetic' was the spec, and I read an instrument list as a genre.** Round two (suite3) is the showcase suite on an orchestra. 🔴 **Dolby caps at 5.1 in ffmpeg and Atmos needs the licensed encoder** · 🔴 **gates assert CLAIMS comparatively, not just ranges**
