...
export function figureSvg(pose, { detail = 'full', title = '', className = '', mix = 0, sk, equip } = {}) {
  ...
  const inner = mark
    ...
    : [
      ...
      // Round 4: a depth-tagged BEHIND-the-body item dims — full saturation
      // under the body's translucent fill read as a dark hole (the 180° bell).
      `<g fill="var(--equipment)"${equip ? ' opacity="0.55"' : ''}>${equipMarkup(f.equip)}</g>`,
