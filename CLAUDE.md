...
## The figures — law (B2, shipped v2.22–v2.24.1; thread: `HANDOFF-figures.md`)
...
- **v2.21–v2.24.1** (08-22) — the movement library, then the figures: the pilot pair → all 340 from 44 patterns, rotation, zoom, direct gestures, colour roles, fault muscles on 8. → `v2.21`…`v2.24.md`
...
- **The 2026-08-22 brief:** the **figures' next round runs in the CCHealth session** (`HANDOFF-figures.md` — this session must NOT touch `src/figures/*`, `Figure.jsx`, `MovementSheet.jsx`, `figureText.js`); the design round + logo shipped as v2.25. Brief rulings that are LAW: 🔴 **the movement card FITS, never scrolls** · 🔴 **the fault figure highlights DIFFERENT muscles**. Open from B3: Pierre picks the mark — round 2 is the FACING PAIR, his direction ("> <", correct mirrored left, fault right; swap = `--freeze <id>` + rebuild) — then Play/iOS icons + favicon derive from it, then an ANIMATED mark + a THX/Atmos-style spatial opening sound (native stage; web autoplay blocks it).
