...
**Store state** — 🔴 NEVER report Play/App Store status from a HANDOFF, a doc or memory. Open the console and read it (`play.google.com/console/u/1/developers` → the u/ index matters: the Play account is **pierreghorra@**, Chrome's default profile is pierreishere@). 2026-08-21 cost a full day of the 14-day clock: the handoff said "in review", the release had been live since Aug 20, and the real blocker was `0 testers opted-in`. A tester on the email list has NOT opted in — Google emails nobody; the link is sent by hand.
