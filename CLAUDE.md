...
Mobile-first app for a personal trainer's gym clients. The PT uses it on his iPhone; Pierre develops and tests on Android.
...
Elie may drive app changes in-session on Pierre's conditions (rollback via git, backups at every juncture — his words in `docs/elie-next-visit.md`).
...
- **v2.37–v2.42** (08-23) — Big Steps type, Display sheet, Judging Bench; swipe-week (NO setPointerCapture: tappable children); `--trench` + the `sanity-contrast` gate; `--on-bar` + radius tokens; 21 skins = 7 families × light/optimal/dark, flanks GENERATED by `gen-skin-variants.mjs`; pebble→FLINT. → `v2.37`…`v2.42`
- **≤v2.36** — `changelog-summary.md`. rollback `snapshot-pre-v2.9.5`.
...
- **P3 — BUILT (v2.25); its Dashboard-compact follow-up MOOT.** `com.spotset.app` is PERMANENT.
- 🔴 **SUPABASE IS PHASE 1 — THE DAILY JOB IS `node scripts/soak-day.mjs`** (bare-diff caveat in the deploy notes). → `HANDOFF-multi-user-build.md` §0.
...
- 🔴 **FRESH EYES, STRIPPED-STRUCTURE (standing rule):** before a design is called done, a subagent that has never seen it — Fable 5, max effort, ALL formatting stripped — argues the opposite. 🔴 **BUILD THE BRIEF FROM OPENED FILES, NEVER FILENAMES** — a guess in a blind brief becomes a confident finding (`traps.md`, the plate calculator that never existed). Runs: `docs/design/2026-08-22-fresh-eyes-*-review.md`.

---

## REVIEW DISCIPLINE
After **3+ feature changes** or ~2 h of coding, pause: fix landed everywhere the pattern exists? · every read AND write migrated on a storage refactor? · callbacks shadowing `t`/`d`? · inline `marginLeft`/`borderLeft`, hardcoded colours? · strings missing from `i18n.js`? · deletes/overwrites/unmigrated data? · new `.catch(() => {})` or loop dispatches? (`docs/release-hygiene.md` §3.)
...
# 6. Deploy to gh-pages (MAKES IT LIVE) — via the STANDING WORKTREE, never
#    `git checkout gh-pages`. Core five + audio (v2.31).
...
**Never skip these "just this once" — that is how the file regrew 19.5→42 KB once already.** Why: `docs/release-hygiene.md` §1.
