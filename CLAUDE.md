...
**THE LOGIN IS LIVE + THE MARK CARRIES THE SPOT.** 🔴 **`.env` IS A BUILD INPUT — no `.env` (VITE_SUPABASE_URL + ANON key, values in `_archive/PTApp/supabase-spotset.env`) builds a token-only app with the whole auth surface dark. ANON KEY ONLY; the service_role key never enters this repo or bundle.** Entry = email sign-in (accounts PROVISIONED in the Supabase console — **no self-signup, Apple 4.8**) · "Continue as guest" (the DEMO path + its refusal gate, button not magic word) · the token field until the parallel run ends. A signed-in tester gets an EMPTY workspace under `ptapp-data:<userId>` — never live data. **The Spot** = a `var(--accent)` dot above the pair (`pm-spot`: wanders in, settles); the S-from-the-pair rounds are PARKED in `_archive/PTApp/branding/`. → `v2.43`…`v2.43.2`.
