...
- 🔴 **TODO — THREE SPECIFIED, UNSTARTED ROUNDS, ALL IN `docs/2026-08-22-figures-next-round-brief.md`** (Pierre, 08-22, in his words): the **figures' next round** (6 items), a **design refinement round at xhigh**, and **the logo (B3)** — name stays, mark open, drawn FROM the figure library. Two rulings from it are LAW and must not be re-litigated: 🔴 **the movement card is to FIT, not scroll** (which frees the figure to take `touch-action: none` and own pinch + drag), and 🔴 **the fault figure highlights DIFFERENT muscles** — a different posture recruits differently, reframing the pair from right-vs-wrong to this-trains-X vs that-trains-Y.
