...
## Version History
- **v2.39–v2.42** (08-23) — swipe-week (NO setPointerCapture: the strip has tappable children); rounded rects + `--trench` separators + the `sanity-contrast` gate (a screenshot finding is audited in EVERY skin); `--on-bar` + radius tokens; 21 skins = 7 families × light/optimal/dark, flanks GENERATED by `gen-skin-variants.mjs`, never hand-edited; pebble→FLINT. → `v2.39`…`v2.42`
- **v2.37–v2.38** (08-23) — Big Steps type, six skins, the Display sheet; the Judging Bench. → `v2.37`, `v2.38`
- **v2.17–v2.25.3** (08-21/22) — the design pass, the movement library, the figures. → `v2.17`…`v2.25.3`
- **v2.26–v2.36** (08-22/23) — logo, opening + sound, turns, the Library tab; NEEDLE; the legibility pass. → `v2.26`…`v2.36`
