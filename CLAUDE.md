...
**THE REVIEW FIXES.** 🔴 **`.env` IS A BUILD INPUT — no `.env` (VITE_SUPABASE_URL + ANON key, values in `_archive/PTApp/supabase-spotset.env`) builds a token-only app with the auth surface dark; ANON KEY ONLY, service_role never enters repo or bundle.** Entry = PROVISIONED email sign-in (no self-signup, Apple 4.8) · guest/DEMO · the token field until the parallel run ends; a signed-in tester gets an EMPTY workspace, never live data — **`isSignedIn()` gates EVERY PAT path**. v2.46 closed the 2026-08-25 review's 12 confirmed findings; the durable rules are in TRAPS. Deferred: D4 (packages LWW). → `v2.46`, `docs/reviews/2026-08-25-full-app-review.md`.
...
- 🔴 **A MOVEMENT IS NOT THE UNIT OF DRAWING — A PATTERN IS.** 44 archetypes (`figures/archetypes.js`) own the pose and the fault; the movement adds **muscles from the bank** and **equipment from its NAME**. `classify.js` maps all 340, first-match-wins, **100% coverage is a build gate** — unclassified silently shows nothing. 🔴 Its ORDER is the logic: "external rotation" must beat "rotation".
...
- 🔴 **SAY WHAT THE POSITION DOES, NEVER WHAT IT CAUSES.** `figureText.js` is keyed by PATTERN (44 entries — what makes Elie's review possible). **No named pathology, no evidence-grade adverb**; the build FAILS on either. **The Arabic may never be stronger than the English.** 🔴 **A REVIEW FLAG NOTHING RENDERS IS NOT A REVIEW PROCESS** — every entry is `reviewed: false` and the panel prints it.
- 🔴 **THE FIGURE'S MEANINGS ARE ALL DERIVED.** Posture line = `--ok` held / `--warn` at fault (v2.24 ruling: green held · blue equipment · orange stress); muscle bands = `--muscle` prime / `--muscle-2` supporting; **`--anatomy` is the fault marker and nothing else**; `--equipment` is the gear. All figure-internal and UI-forbidden; `sanity-skins` asserts every token per skin. A figure carrying this many meanings **ships its key**.
- 🔴 **A PATTERN MAY ROTATE (`ROTATES` in `poses.js`) — A TWEEN BETWEEN TWO AUTHORED CAMERAS, NOT A 3D RIG**, so it cannot turn into a blob. **Both halves turn together** (turning one compares two cameras and teaches the wrong difference); **equipment SWAPS at 0.5**, never tweens; a rotatable pattern DROPS its third figure. 🔴 True 3D was tried and reverted — a world-fixed lateral axis breaks every supine pose; it needs a BODY-FIXED frame per segment. Gesture: pointer events + `touch-action: pan-y`, **never a non-passive preventDefault**.
- **A THIRD FIGURE (`extraId`) IS FOR A FAULT OUTSIDE THE PAIR'S PLANE** — its own camera, marker and sentence. 🔴 **NEVER fake it with per-figure foreshortening**: a bone-length change disguised as perspective.
- **Bilingual search folds through `normaliseSearch` (`utils.js`) on BOTH sides** — typed Arabic differs from written invisibly and `includes()` fails. `muscleLabel`/`MUSCLE_AR` own muscle names; **`t()` returns the KEY on a miss — never use it for data**.
...
- **`TOKEN_KEY`/`DEMO_TOKEN`/`isDemo` in `utils.js`** (import cycle). 🔴 **`DEMO` is a review credential** — seeded local data, sync off, **ONLY onto an empty store**.
...
## Version History
- **v2.43–v2.45** (08-23/24) — login live (email + guest + token) + the Spot; two anatomy passes; vc25 to Play Alpha. → `v2.43`…`v2.45`
- **v2.37–v2.42** (08-23) — Big Steps type, the Display sheet, the Judging Bench; swipe-week (NO setPointerCapture: the strip has tappable children); rounded rects + `--trench` + the `sanity-contrast` gate (a screenshot finding is audited in EVERY skin); `--on-bar` + radius tokens; 21 skins = 7 families × light/optimal/dark, flanks GENERATED by `gen-skin-variants.mjs`, never hand-edited; pebble→FLINT. → `v2.37`…`v2.42`
- **v2.17–v2.36** (08-21/23) — design pass, movement library, figures; logo, opening + sound, turns, Library tab; NEEDLE; legibility. → `v2.17`…`v2.36`
- **v2.15 and earlier** — `changelog-summary.md`. Rollback tag: `snapshot-pre-v2.9.5`.
...
**iOS / mobile** — 🔴 **EVERY MODAL PORTALS TO `<body>`** (`.content`'s bounce transform re-anchors `position:fixed`; its `z-index:1` puts overlays under the nav) **and `dir` is on `<html>`** or portalled Arabic renders LTR · 🔴 **an unpersisted toggle is a tax, not a preference** — only whoever wanted the other option ever finds it · safe-area insets, modal z-index 200+, sticky `modal-footer`, visualViewport resize · tap targets in the bottom 60% · never start a textarea `readOnly` and clear it in `onFocus` · swipe-to-dismiss only when `scrollTop === 0` at touchstart · PWA standalone needs BOTH the meta tag AND `manifest.json` `"display":"standalone"`.
...
**Figures at scale (v2.23)** — a supine/prone figure needs its centre line one torso-depth off the floor and a plank needs its **shoulders higher than its heels**, or it reads as someone lying down · an ORDERED classifier fails SILENTLY when misordered — it just draws the wrong movement · a LATER rule's keyword is DEAD when an earlier regex substring-matches (Arnold) — **diff all 340 after any classifier edit** · a gesture-owning element joins Modal's dismiss exclusions.
...
**Tooling** — 🔴 the v2.31 opening suite lives in TWO scripts (`make-opening-suite.mjs` + `-51.mjs`) kept in step BY HAND — edit both; **suite 2 fixed this by DERIVING: one arrangement → 7.1, folded to 5.1 + stereo** · 🔴 **`ac3`/`eac3`/`truehd` all STOP AT 5.1, and Atmos needs the licensed encoder** — ask `-h encoder=X`, never assume a codec's spec · **loudness is RMS, not peak** · 🔴 a python heredoc writing `\b` into a JS regex writes a **BACKSPACE (0x08)**, not a word boundary: the regex parses, runs, and silently stops matching (`cat -A` shows `^H`). Bit twice — use `chr(92)+'b'`, a raw string, or the Edit tool · 🔴 the Android build needs **JDK 21** (`JAVA_HOME='/c/Program Files/Microsoft/jdk-21.0.12.8-hotspot'`); PATH java here is Temurin 8, and **`gradlew` exits 0 on a FAILED build**, leaving the previous AAB in `outputs/` — a stale bundle then uploads and Play rejects it as "version code already used". Verify the versionName INSIDE the .aab, never the exit code · 🔴 **`gradlew` WITHOUT `npx cap sync` ships the PREVIOUS web bundle under the NEW versionName** — only `unzip -p <apk> assets/public/index.html` proves what the tester sees · 🔴 **a new colour token must be added to `LIT` in `logo-candidates.mjs --export`** or every icon renders it unresolved · `fixForFileProtocol` must use *function* replacement (a string breaks React's minified `$&`) · **PS 5.1 `Get-Content`/`Set-Content` mangles UTF-8** (ANSI default corrupts em-dashes, emoji and all Arabic) — use the **Edit tool**, never round-trip source through a PowerShell pipeline.
...
- **P3 — BUILT (v2.25); its Dashboard-compact follow-up MOOT** (v2.33.1 deleted that view).
- `com.spotset.app` is PERMANENT.
- 🔴 **SUPABASE IS PHASE 1 — THE DAILY JOB IS `node scripts/soak-day.mjs` (mirror then verify).** The app does NOT dual-write, so a bare `sanity-live-supabase-diff` can never be clean; it becomes the real soak at dual-write. → `HANDOFF-multi-user-build.md` §0.
...
- 🔴 **FRESH EYES, STRIPPED-STRUCTURE (standing rule):** before a design is called done, a subagent that has never seen it — Fable 5, max effort, ALL formatting stripped — argues the opposite. 🔴 **BUILD THE BRIEF FROM OPENED FILES, NEVER FILENAMES** — a guess in a blind brief becomes a confident finding (`traps.md`, the plate calculator that never existed). Runs: `docs/design/2026-08-22-fresh-eyes-structure-review.md` · `…-navigation-review.md`.
