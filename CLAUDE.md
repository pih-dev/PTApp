...
**THE LOGIN IS LIVE.** 🔴 **`.env` IS A BUILD INPUT — no `.env` (VITE_SUPABASE_URL + ANON key, values in `_archive/PTApp/supabase-spotset.env`) builds a token-only app with the whole auth surface dark. ANON KEY ONLY; the service_role key never enters this repo or bundle.** Entry = email sign-in (accounts PROVISIONED by Pierre — no self-signup, Apple 4.8) · "Continue as guest" (the DEMO path + its refusal gate, button not magic word) · the token field until the parallel run ends. A signed-in tester gets an EMPTY workspace under `ptapp-data:<userId>` — never live data. → `v2.43`.
...
## Version History
- **v2.42** (08-23) — 21 skins (7 families × light/optimal/dark; flanks GENERATED by `gen-skin-variants.mjs`, never hand-edited); pebble→FLINT; buttons 40px/7px; picker = family cards + variant row. → `v2.42`
