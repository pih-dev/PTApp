...
**Branding only (2026-08-20): the app is named SpotSet in the UI.** `DATA_VERSION` stays 6, no migration, no kernel or reducer touched. Detail: `docs/instructions-v2.15.0.md`.
- **The header, PWA manifest, page title and iOS web-app title all say SpotSet**; "PTApp" is now only the repo/project name. Caught because the Play store listing said SpotSet while every screen said PTApp.
- **Launcher icons are the SpotSet barbell mark**, generated by `_archive/PTApp/branding/make_icon.py`. 🔴 The adaptive foreground must stay inside the **66% safe zone** — Android crops 25% per edge and a full-bleed foreground clips the plates.
- **`versionCode 2` / `versionName 2.15.0`**, uploaded to the Play closed-testing track. Store state, every declaration filed with Google, and the traps: `HANDOFF-spotset-publishing.md` §6–§7.
